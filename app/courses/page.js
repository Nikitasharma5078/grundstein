...
export default function CoursesPage() {
  const bands = [
    { name: "Grundstufe", key: "grundstufe", levels: ["a1", "a2"] },
    { name: "Aufbaustufe", key: "aufbaustufe", levels: ["b1", "b2"] },
    { name: "Fortgeschritten", key: "fortgeschritten", levels: ["c1"] },
  ];

  return (
    <div className="max-w-6xl mx-auto px-6 py-16">
      <p className="mono text-xs text-red mb-3 tracking-widest">ALLE KURSE</p>
      <h1 className="display text-4xl font-semibold mb-12">Choose your level</h1>

      {bands.map((band) => (
        <div key={band.key} className="mb-14">
          <h2 className="mono text-sm text-ink-soft mb-4 uppercase tracking-wide">{band.name}</h2>
              ...
              return (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="mono text-xs text-blue">{c.lessons.length} lessons →</p>
                  </div>
                </a>
              );
            })}
